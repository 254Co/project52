...
from chen3.datatypes import EquityParams, ModelParams, RateParams
...
def test_european_call_pricing():
    ...
    model = Chen3Model(model_params)
    option = Vanilla(strike=100.0, call=True)
...
def test_american_put_pricing():
    ...
    model = Chen3Model(model_params)
    option = Vanilla(strike=100.0, call=False)
...
def test_asian_call_pricing():
    ...
    model = Chen3Model(model_params)
    option = Asian(strike=100.0, call=True)
...
def test_correlation_structure():
    ...
    rate_equity_corr = np.corrcoef(rate_returns.flatten(), equity_returns.flatten())[
        0, 1
    ]
    ...
    if np.any(mask):
        equity_var_corr = np.corrcoef(
            equity_returns.flatten()[mask], var_returns.flatten()[mask]
        )[0, 1]
