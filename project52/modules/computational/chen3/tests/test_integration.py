...
def test_full_pricing_pipeline():
    ...
    options = [
        ...
        CallablePuttable(obs_times=[100], call_strike=0.0, put_strike=100.0), # AmericanPut (approx)
        Asian(strike=100.0, call=True),    # AsianCall
        Barrier(strike=100.0, barrier=110.0, knock_in=False, call=True) # BarrierOption (up-and-out call)
    ]
    ...
    put_price = prices[1]
    expected_diff = (
        equity_params.S0 * np.exp(-equity_params.q * 1.0) -
        100.0 * np.exp(-rate_params.r0 * 1.0)
    )
