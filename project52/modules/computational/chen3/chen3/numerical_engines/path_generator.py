...
class PathGenerator:
    ...
    def _generate_fixed_paths(
        ...
        dW = self.rng.generate_normal(
            size=(n_paths, n_steps, 3), correlation_matrix=correlation_matrix
        )
        ...
        for t in range(n_steps):
            ...
            (
                ...
                equity_paths[:, t + 1],
                variance_paths[:, t + 1],
            ) = self.integration_scheme.step(state, drift, diffusion, dW[:, t], dt)
    ...
    def _generate_adaptive_paths(
        ...
        while step < n_steps:
            ...
            # Generate random numbers
            dW = self.rng.generate_normal(
                size=(n_paths, 3), correlation_matrix=correlation_matrix
            )

            # Compute error estimate
            error = self._estimate_error(state, drift, diffusion, dW, dt)
            ...
            ) = self.integration_scheme.step(state, drift, diffusion, dW, dt)
    ...
    def _estimate_error(
        ...
        state_half = self.integration_scheme.step(state, drift, diffusion, dW, dt / 2)

        # Compute one step with dt
        state_full = self.integration_scheme.step(state, drift, diffusion, dW, dt)

        # Compute error estimate
        error = np.max(np.abs(np.array(state_half) - np.array(state_full)))
