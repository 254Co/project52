...
from jax import grad, jacfwd, jacrev, random, vmap
...
def multi_asset_price_and_greeks_jax(
    ...
    def single_path(key):
        def step(logS, subkey):
            z = random.multivariate_normal(
                jnp.zeros(n_assets), correlation, shape=(1,)
            )[0]
    ...
    # Compute second-order Greeks
    gamma = jnp.array(
        jacfwd(jacrev(price_fn, argnums=0), argnums=0)(S0, r, sigma, T, K)
    )
    vanna = jnp.array(
        jacfwd(jacrev(price_fn, argnums=0), argnums=2)(S0, r, sigma, T, K)
    )
    volga = jnp.array(
        jacfwd(jacrev(price_fn, argnums=2), argnums=2)(S0, r, sigma, T, K)
    )
