...
class GPUMonteCarloEngine(MonteCarloEngineBase):
    ...
    def _load_kernels(self):
        ...
        try:
            ...
            self.simulate_paths_kernel = cp.RawKernel(r'''
            extern "C" __global__
            void simulate_paths(
                float* rate_paths,
                float* equity_paths,
            ...
            self.compute_payoff_kernel = cp.RawKernel(r'''
            extern "C" __global__
            void compute_payoff(
                float* payoffs,
            ...
            self.variance_reduction_kernel = cp.RawKernel(r'''
            extern "C" __global__
        ...
        try:
            ...
            self.simulate_paths_kernel(
                ...
                (
                    ...
                    variance_paths_gpu,
                    dW_gpu,
                    cp.array([initial_state['r'], initial_state['S'], initial_state['v']]),
                    self.dt,
                    self.n_steps
                )
            )
            ...
            if self.use_antithetic:
                rate_paths_gpu = (rate_paths_gpu[:self.n_paths] + rate_paths_gpu[self.n_paths:]) / 2
                equity_paths_gpu = (equity_paths_gpu[:self.n_paths] + equity_paths_gpu[self.n_paths:]) / 2
                variance_paths_gpu = (variance_paths_gpu[:self.n_paths] + variance_paths_gpu[self.n_paths:]) / 2
