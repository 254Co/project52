    ...
    def simulate_paths(
        ...
        try:
            ...
            for t in range(self.n_steps):
                ...
                safe_r = np.maximum(state["r"], 1e-10)  # Prevent sqrt(0)
                safe_v = np.maximum(state["v"], 1e-10)  # Prevent sqrt(0)
                ...
                rate_paths[:, t + 1] = np.maximum(rate_paths[:, t + 1], 1e-10)

                # Update equity paths (Geometric Brownian Motion with stochastic volatility)
                log_S = np.log(np.maximum(equity_paths[:, t], 1e-10))
                drift_term = np.clip(drift[1] * self.dt, -10.0, 10.0)
                diffusion_term = np.clip(diffusion[1] * dW[:, t, 1], -10.0, 10.0)
                log_S_new = log_S + (drift_term - 0.5 * np.clip(diffusion[1]**2 * self.dt, 0.0, 10.0)) + diffusion_term
