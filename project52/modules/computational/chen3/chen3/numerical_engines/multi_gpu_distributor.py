...
class MultiGPUDistributor:
    ...
    def distribute_task(
        self, task_type: str, task_params: Dict
    ) -> Union[cp.ndarray, List[cp.ndarray]]:
        """
        ...
        self.current_task = {"type": task_type, "params": task_params, "id": task_id}
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for i, gpu_id in enumerate(self.settings.gpu_ids):
                ...
                future = executor.submit(
                    self._run_mc_task_with_retry, gpu_id, gpu_params
                )
    ...
    def _run_mc_task_with_retry(
        self, gpu_id: int, params: Dict, max_retries: int = 3
    ) -> cp.ndarray:
        """
        ...
        while retry_count < max_retries:
            try:
                ...
                self.task_metrics[params["task_id"]].paths_completed += params[
                    "n_paths"
                ]
                return result
    ...
    def _distribute_pde_task(self, params: Dict) -> List[cp.ndarray]:
        ...
        with ThreadPoolExecutor(max_workers=n_gpus) as executor:
            ...
            for i, gpu_id in enumerate(self.settings.gpu_ids):
                ...
                future = executor.submit(self._run_pde_task, gpu_id, gpu_params)
                futures.append(future)
    ...
    def _optimize_mc_task(self, params: Dict) -> Dict:
        ...
        optimal_batch = min(
            params["n_paths"], available_memory // (bytes_per_path * params["n_steps"])
        )
            ...
            with memory_manager.allocate():
                ...
                with performance_monitor.monitor():
                    ...
                    checkpoint = checkpoint_manager.load_checkpoint(
                        task_id=params.get("task_id"), task_type="mc"
                    )
        ...
        try:
            ...
            with memory_manager.allocate():
                ...
                with performance_monitor.monitor():
                    # Check for checkpoint
                    checkpoint = checkpoint_manager.load_checkpoint(
                        task_id=params.get("task_id"), task_type="pde"
                    )
    ...
    def _run_new_mc_task(self, params: Dict) -> cp.ndarray:
        ...
        if params.get("batch_size"):
            results = []
            n_batches = (params["n_paths"] + params["batch_size"] - 1) // params[
                "batch_size"
            ]
            ...
            for i in range(n_batches):
                ...
                if (
                    params.get("checkpoint_interval")
                    and (i + 1) % params["checkpoint_interval"] == 0
                ):
                    state = sim.get_state()
    ...
    def _fuse_pde_kernels(self, solver: GPUPDESolver, params: Dict) -> cp.ndarray:
        ...
        for t in range(solver.total_steps):
            ...
            if (
                params.get("checkpoint_interval")
                and (t + 1) % params["checkpoint_interval"] == 0
            ):
...
class DistributedMonteCarloEngine(MonteCarloEngineBase):
    ...
    def simulate_paths(
        ...
        try:
            ...
            # Update task metrics
            self.task_metrics[self.current_task_id] = self.distributor.get_task_metrics(
            ...
            if self.current_task_id:
                self.task_metrics[self.current_task_id] = (
                    self.distributor.get_task_metrics(self.current_task_id)
                )
    ...
    def compute_expectation(
        ...
        try:
            ...
            raise NumericalError(
                f"Distributed expectation computation failed: {str(e)}"
            )
