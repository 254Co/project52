            ...
            for i in range(self.n_factors):
                for j in range(self.n_factors):
                    if i == j:
                        ...
                        corr[i, j] = weights[i, j] * np.exp(
                            -paths[i, j] / self.decay_param
                        )
...
def create_network_correlation(
    n_factors: int = 3, weight_type: str = "constant", decay_rate: float = 0.1
) -> NetworkCorrelation:
    """
    ...
    if weight_type == "constant":
        ...
        weight_function = lambda t, s: np.exp(-decay_rate * t) * np.ones(
            (n_factors, n_factors)
        )
    elif weight_type == "oscillate":
        weight_function = lambda t, s: (1 + np.cos(decay_rate * t)) * np.ones(
            (n_factors, n_factors)
        )
    else:
