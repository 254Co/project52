...
class DynamicCorrelation(BaseCorrelation):
    ...
    def __init__(
        ...
        self.weight_functions = weight_functions or [
            lambda t, s: 1.0 / len(correlation_structures)
        ] * len(correlation_structures)
        self._validate_initialization()
        logger.debug(
            f"Initialized {self.name} with {len(correlation_structures)} structures"
        )
    ...
    def _validate_initialization(self):
        ...
        if not self.correlation_structures:
            raise CorrelationValidationError(
                "At least one correlation structure is required"
            )
    ...
    def _compute_weights(
        ...
        """
        try:
            weights = np.array([w(t, state) for w in self.weight_functions])
    ...
    def __str__(self) -> str:
        ...
        return f"{self.name}(" f"n_structures={len(self.correlation_structures)})"
...
def create_dynamic_correlation(
    ...
    if weight_type == "equal":
        weight_functions = [lambda t, s: 1.0 / len(correlation_structures)] * len(
        ...
        weight_functions = [
            lambda t, s: (
                1.0 / len(correlation_structures)
                if s is None
                else np.exp(-0.1 * abs(s.get("rate", 0.0)))
                / len(correlation_structures)
            )
        ] * len(correlation_structures)
    else:
