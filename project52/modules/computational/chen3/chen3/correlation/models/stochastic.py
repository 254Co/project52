...
class StochasticCorrelation(BaseCorrelation):
    ...
    def _validate_initialization(self):
        ...
        """
        # Validate shapes
        if not all(x.shape == (self.n_factors, self.n_factors) for x in [
            self.mean_reversion,
            self.long_term_mean,
            self.volatility,
            self.initial_corr
        ...
        if not np.all(np.abs(self.long_term_mean) <= 1):
            raise CorrelationValidationError("Long-term mean correlations must be in [-1, 1]")
        
        # Validate volatility
        if not np.all(self.volatility >= 0):
            raise CorrelationValidationError("Correlation volatilities must be non-negative")
    ...
    def _simulate_correlation(
        ...
        try:
            ...
            for _ in range(n):
                ...
                dW = np.random.normal(0, np.sqrt(dt), size=(self.n_factors, self.n_factors))
    ...
    def __str__(self) -> str:
        ...
        return (
            f"{self.name}("
            f"mean_reversion={self.mean_reversion.mean():.2f}, "
            f"volatility={self.volatility.mean():.2f})"
        )
