...
def test_factory_functions():
    ...
    assert corr.copula_type == "gaussian"
    assert np.allclose(corr.correlation_matrix, np.array([
        [1.0, 0.6, 0.6, 0.6],
        [0.6, 1.0, 0.6, 0.6],
        [0.6, 0.6, 1.0, 0.6],
        [0.6, 0.6, 0.6, 1.0]
    ...
    # Test Student's t copula
    corr = create_student_copula_correlation(
        base_corr=0.6,
        df=5.0,
        n_factors=4
    )
    ...
    # Test Clayton copula
    corr = create_clayton_copula_correlation(
        base_corr=0.6,
        theta=2.0,
        n_factors=4
    )
    ...
    # Test Gumbel copula
    corr = create_gumbel_copula_correlation(
        base_corr=0.6,
        theta=2.0,
        n_factors=4
    )
