...
def test_validation():
    ...
    def invalid_corr_func(state):
        return np.array([[1.0, 1.5, 0.3], [1.5, 1.0, 0.2], [0.3, 0.2, 1.0]])

    with pytest.raises(CorrelationValidationError):
        StateDependentCorrelation(invalid_corr_func)

    def non_symmetric_corr_func(state):
        return np.array([[1.0, 0.5, 0.3], [0.6, 1.0, 0.2], [0.3, 0.2, 1.0]])

    with pytest.raises(CorrelationValidationError):
        StateDependentCorrelation(non_symmetric_corr_func)

    def non_positive_definite_corr_func(state):
        return np.array([[1.0, 0.9, 0.9], [0.9, 1.0, 0.9], [0.9, 0.9, 1.0]])
...
def test_string_representations():
    ...
    expected_str = (
        f"StateDependentCorrelation(" f"default_state={{'r': 0.0, 'S': 0.0, 'v': 0.0}})"
    )
