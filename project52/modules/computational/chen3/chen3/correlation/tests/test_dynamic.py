...
def test_initialization():
    ...
    weight_funcs = [lambda t, s: 0.7, lambda t, s: 0.3]
    dynamic_corr = DynamicCorrelation(
        [corr1, corr2], weight_functions=weight_funcs, n_factors=3, name="CustomDynamic"
    )
    ...
    with pytest.raises(CorrelationValidationError):
        DynamicCorrelation(
            [corr1], weight_functions=[lambda t, s: 0.5, lambda t, s: 0.5]
        )
...
def test_correlation_matrix():
    ...
    weight_funcs = [lambda t, s: np.exp(-0.1 * t), lambda t, s: 1.0 - np.exp(-0.1 * t)]
