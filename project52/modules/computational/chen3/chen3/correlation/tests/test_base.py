...
class TestCorrelation(BaseCorrelation):
    ...
    def get_correlation_matrix(self, t: float = 0.0, state: dict = None) -> np.ndarray:
...
def test_correlation_matrix_validation():
    ...
    with pytest.raises(CorrelationError):
        corr._validate_correlation_matrix(
            np.array([[1.0, 0.5, 0.3], [0.6, 1.0, 0.2], [0.3, 0.2, 1.0]])
        )

    # Test invalid diagonal
    with pytest.raises(CorrelationError):
        corr._validate_correlation_matrix(
            np.array([[1.0, 0.5, 0.3], [0.5, 0.9, 0.2], [0.3, 0.2, 1.0]])
        )

    # Test invalid correlation values
    with pytest.raises(CorrelationError):
        corr._validate_correlation_matrix(
            np.array([[1.0, 1.5, 0.3], [1.5, 1.0, 0.2], [0.3, 0.2, 1.0]])
        )
...
def test_correlated_random_generation():
    ...
    with pytest.raises(CorrelationError):
        corr._generate_correlated_random(
            n_paths, np.array([[1.0, 0.9, 0.9], [0.9, 1.0, 0.9], [0.9, 0.9, 1.0]])
        )
