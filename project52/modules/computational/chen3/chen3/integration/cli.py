...
from chen3.payoffs import Vanilla, Barrier, Asian, ConvertibleBond
...
@cli.command()
@click.option('--model-params', type=click.Path(exists=True), required=True,
              help='JSON file with model parameters (rate, equity, corr).')
@click.option('--product', type=click.Choice(list(PRODUCT_MAP.keys())), required=True,
              help='Product type to price.')
@click.option('--payoff-params', type=click.Path(exists=True), required=True,
              help='JSON file with payoff parameters (strike, barrier, etc.).')
@click.option('--settings', type=click.Path(exists=True), required=True,
