...
class MertonJump:
    ...
    def __init__(self, lam: float, mu_j: float, sigma_j: float):
        ...
        if lam < 0 or sigma_j < 0:
            raise ValueError(
                "Jump intensity and standard deviation must be non-negative"
            )
    ...
    def simulate(
        ...
        jumps = np.exp(rng.normal(self.mu_j, self.sigma_j, size=(n_paths, n_steps)) * N)
        return jumps
...
class KouJump:
    ...
    def __init__(self, lam: float, p: float, eta1: float, eta2: float):
        """
